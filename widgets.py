...
class SettingsWidget(QGroupBox):
    def __init__(self):
        ...
        layout.addRow("Папка для наблюдения:", self.folder_layout)

        # Поле для URL репозитория
        self.repo_edit = QLineEdit()
        layout.addRow("URL репозитория:", self.repo_edit)
        ...
        layout.addRow("Качество:", self.quality_spin)
        ...
        layout.addRow("Макс. размер:", self.max_size_spin)

        # Загрузка настроек
        self.load_settings()

    def select_folder(self):
        folder = QFileDialog.getExistingDirectory(self, "Выберите папку для наблюдения")
        if folder:
    ...
    def load_settings(self):
        settings = QSettings("YourCompany", "ImageBackupTool")
        self.folder_edit.setText(settings.value("watch_folder", ""))
        self.repo_edit.setText(settings.value("repo_url", ""))
        self.format_combo.setCurrentText(settings.value("compression_format", "WebP").capitalize())
